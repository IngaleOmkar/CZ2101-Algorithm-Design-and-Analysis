...
def writeResults(fileName, iter, d):
    with open(fileName, 'w', newline='') as f:
        ...
        try:
            ...
            print("Process stopped by user... Saved current results")
            return
...
if __name__ == "__main__":
    ...
    for i in range(START, ITERATIONS+1):
        try:
            ...
            # G.randomLinearGraph() # Generate random linear graph (like a linked list) [for best case] -> uncomment to use
            
            d["Number of Vertices"].append(i)
            
            # Getting time for adjMatrix with priority queue array
            start = time()
            ...
            # Getting time for adjList with min heap
            start = time()
